...
eval val@(Integer   _                  ) = pure val
eval val@(Bool      _                  ) = pure val
eval val@(Character _                  ) = pure val
eval (    List      (Atom _ : [])      ) = throwError . show $ NumArgs 1 []
...
numericBoolOp
  :: (Integer -> Integer -> Bool) -> [LispVal] -> ThrowsError LispVal
numericBoolOp _  [Integer _     ] = pure $ Bool True
numericBoolOp op (Integer x : xs) = case head xs of
  Integer y -> if x `op` y then numericBoolOp op xs else pure $ Bool False
  notNum    -> throwError . show $ TypeMismatch "number" notNum
numericBoolOp _ _ =
  throwError . show $ TypeMismatch "number" $ String "a list of non numbers"
...
numericOp _  []     = throwError . show $ NumArgs 1 []
numericOp op params = Integer . foldl1 op <$> mapM unpackNum params
 where
  unpackNum (Integer n) = pure n
